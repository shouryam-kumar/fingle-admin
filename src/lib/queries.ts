...
export async function getAllUsers(limit = 50, offset = 0) {
  const { data } = await supabase
    .from('users')
    .select('id, auth_id, name, username, email, profile_pic, is_verified, is_suspended, suspension_reason, created_at, followers_count, following_count')
    .order('created_at', { ascending: false })
...
export async function getUserDetails(userId: string) {
  const { data } = await supabase.rpc('admin_get_user_details', { p_user_id: userId });
  return data;
}

export async function getUserContent(userId: string, limit = 50, offset = 0) {
  const { data } = await supabase.rpc('admin_get_user_content', { p_user_id: userId, p_limit: limit, p_offset: offset });
  return data ?? [];
}

export async function getReports(status?: string, limit = 50, offset = 0) {
  const { data } = await supabase.rpc('admin_get_reports', { p_status: status ?? null, p_limit: limit, p_offset: offset });
  return data ?? [];
}

export async function suspendUser(userId: string, type: 'temporary' | 'permanent', reason: string, adminId: string, expiresAt?: string) {
...
export async function unsuspendUser(userId: string, adminId: string) {
  const { data } = await supabase.rpc('admin_unsuspend_user', { p_user_id: userId, p_admin_id: adminId });
  return data;
}

export async function deletePost(postId: string, postType: string, reason: string, adminId: string) {
